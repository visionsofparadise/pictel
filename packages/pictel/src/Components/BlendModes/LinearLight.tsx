...
export const linearLight: BlendFormula = (sr, sg, sb, dr, dg, db) => [
	linearLightChannel(dr, sr),
	linearLightChannel(dg, sg),
	linearLightChannel(db, sb),
]
...
export function LinearLight({ opacity = 1, flatten, children, style, ...rest }: LinearLightProps) {
	const effect = useCallback(
		(self: ImageData, behind: ImageData) => blendPixels(self, behind, linearLight),
		[],
	);
