 ...
 * an intermediate element that signals a nested RasterEffect's children wrapper.
 *
 * Post-DOM-collapse, a nested RasterEffect contributes a wrapper div (whose
 ...
 *
 * An element is "directly owned" by the outer raster effect only if no such
 ...
 * so in-progress nested raster effects are not a concern — only resolved ones
